...
constexpr const char* kTopLevelCommands =
    "version update sdk config profile connect disconnect mute unmute away back status server channel client message playback events completion";
...
auto generate(const std::string& shell) -> domain::Result<std::string> {
    if (shell == "bash") {
        return domain::ok(
            ...
            "    playback) COMPREPLY=( $(compgen -W \"status send --help\" -- \"$cur\") ) ;;\n"
